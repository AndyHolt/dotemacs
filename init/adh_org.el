...
;; adapted from https://stackoverflow.com/questions/10295177/is-there-an-equivalent-of-org-modes-b-ignoreheading-for-non-beamer-documents
(defun adh-org-ignore-headline-html (contents backend info)
  "Ignore headline, but export content, for headlines tagged with
'ignore'. This function deals only with html export. See similar
functions for other export backends."
  (when (and (org-export-derived-backend-p backend 'html)
          (string-match "^.*<span class=\"ignore\">.*\n"
                        (downcase contents)))
    (replace-match "" nil nil contents)))

(defun adh-org-ignore-headline-latex (contents backend info)
  "Ignore headline, but export content, for headlines tagged with
'ignore'. This function deals only with latex export. See similar
functions for other export backends."
  (when (and (org-export-derived-backend-p backend 'latex)
          (string-match "^.*\\\\hfill{}\\\\textsc{.*ignore.*\n"
                        (downcase contents)))
    (replace-match "" nil nil contents)))

(add-to-list 'org-export-filter-headline-functions 'adh-org-ignore-headline-html)
(add-to-list 'org-export-filter-headline-functions 'adh-org-ignore-headline-latex)

;; remove "ignore" tagged headings from TOC in html export
(defun adh-org-html-toc-filter-ignore (toc)
  "Remove entries tagged with :ignore: from TOC generated by org-html-toc.

This function used as a filter-return advice for org-html-toc."
  (while (string-match "^<li>.*<span class=\"ignore\">.*</li>\n" (downcase toc))
      (setq toc (replace-match "" nil nil toc)))
  (format "%s" toc))

(advice-add 'org-html--format-toc-headline
            :filter-return
            #'adh-org-html-toc-headline-filter-ignore)
